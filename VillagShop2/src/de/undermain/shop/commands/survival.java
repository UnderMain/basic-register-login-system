		...
		Player p = (Player) sender;
			...
			if(p.hasPermission("ufc.g")) {
				if(args.length == 0) {
					...
					p.sendMessage(Prefix + "§7» Du bist nun im Survival Modus!");
					p.sendMessage(Prefix + "");
					p.sendMessage(Prefix + "§7§m---------------------------");
					...
					if(t != null) {				
						...
						t.sendMessage(Prefix + "§7» Du bist nun im Survival Modus!");
						t.sendMessage(Prefix + "");
						t.sendMessage(Prefix + "§7§m---------------------------");
						t.playSound(t.getLocation(), Sound.ITEM_PICKUP, 1.0F, 1.0F);

						p.sendMessage(Prefix + "§7» Du hast  §a" + args[0] + "§7 in Survival gesetzt!");
						p.playSound(p.getLocation(), Sound.ITEM_PICKUP, 1.0F, 1.0F);
