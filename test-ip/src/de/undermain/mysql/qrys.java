...
public class qrys {
    ...
    public static String produkt(){
		...
		try {
			st = MySQL.getConnection().prepareStatement("SELECT produkt FROM lizenz WHERE lizenz=? AND name=? AND email=?");
			st.setString(1, lizenz);
			st.setString(2, name);
			st.setString(3, email);
			st.execute();
    ...
    public static String bestellnummer(){
		...
		try {
			st = MySQL.getConnection().prepareStatement("SELECT bestellnummer FROM lizenz WHERE lizenz=? AND name=? AND email=?");
			st.setString(1, lizenz);
			st.setString(2, name);
			st.setString(3, email);
			st.execute();
