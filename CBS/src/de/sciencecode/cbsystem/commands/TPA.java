...
public class TPA implements CommandExecutor{
	...
	public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) { 
		Player p = (Player)sender;
				if	(args.length == 0) {
					p.sendMessage(Main.getPrefix() + "§cSyntax: /tpa [Name]");
				}
					...
					if (target == null) {
						...
						if(req.containsKey(target)){
							...
							if(req.get(target).contains(p)){
								...
								p.sendMessage(Main.getPrefix() + "§aDu hast erfolgreich eine TPA Anfrage an §e" + target.getName() + " §ageschickt!");
								
								target.sendMessage(Main.getPrefix() + "§aDu hast eine TPA Anfrage von §e" + p.getName() + " §aerhalten.");
								
								target.sendMessage(Main.getPrefix() + "§aNutze §6/tpa annehmen " + p.getName() + " §8- Um sie anzunehmen");
								
								target.sendMessage(Main.getPrefix() + "§aNutze §6/tpa ablehnen " + p.getName() + " §8- Um sie abzulehnen");
					...
					p.sendMessage(Main.getPrefix() + "§cSyntax: /tpa [Name]");
					p.sendMessage(Main.getPrefix() + "§cSyntax: /tpa annehmen [Name]");
					p.sendMessage(Main.getPrefix() + "§cSyntax: /tpa ablehnen [Name]");
