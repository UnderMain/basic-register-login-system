...
public class CoinsAPI {
    ...
    private static void updateCoins(Player p, int zahl){
		PreparedStatement st = null;
		UUID uuid1 = Bukkit.getPlayer(p.getName()).getUniqueId();
		try {
			st = MySQLL.getConnection().prepareStatement("UPDATE coinsTable SET coins = ? WHERE UUID = ?");
			st.setInt(1, zahl);
    ...
    private static int coins(Player p){
		...
		UUID uuid1 = Bukkit.getPlayer(p.getName()).getUniqueId();
		try {
			st = MySQLL.getConnection().prepareStatement("SELECT coins FROM coinsTable WHERE UUID=?");
	...
	public static int removeCoins(Player p, int zahl) {
		int coins = coins(p);
		if(zahl > coins) {
			return -1;
		}else if(zahl <= coins) {
