...
public class HealCommand implements org.bukkit.command.CommandExecutor
{
  public HealCommand() {}
  
  public boolean onCommand(CommandSender sender, org.bukkit.command.Command cmd, String label, String[] args)
  {
    if (sender.hasPermission("citybuild.heal")) {
      if (args.length == 0) {
        if ((sender instanceof Player)) {
          Player p = (Player)sender;
          p.setHealth(p.getMaxHealth());
          sender.sendMessage(CityBuildCore.instance.translateString(CityBuildCore.ds_msg.getString("message.healyou")));
          ((Player)sender).playSound(((Player)sender).getLocation(), Sound.LAVA_POP, 1.0F, 1.0F);
        } else {
          sender.sendMessage("You must be a Player, to use this Command!");
        }
      } else if ((args.length == 1) && (sender.hasPermission("citybuild.heal.other"))) {
        Player target = org.bukkit.Bukkit.getPlayer(args[0]);
        
        if (target != null)
        {
          target.setHealth(target.getMaxHealth());
          sender.sendMessage(CityBuildCore.instance.translateString(CityBuildCore.ds_msg.getString("message.healother").replace("%PLAYER%", target.getName())));
          if ((sender instanceof Player)) {
