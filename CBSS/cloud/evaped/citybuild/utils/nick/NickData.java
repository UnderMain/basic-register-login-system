...
{
  public static HashMap<String, Object> getObject_nicknames = new HashMap();
  public static HashMap<String, Object> getObject_nicksettings = new HashMap();
  
  public NickData() {}
  
  public void loadData_NickNames() { File file = new File("plugins/CityBuild/Inventory/", "nicknames.yml");
    FileConfiguration cfg = org.bukkit.configuration.file.YamlConfiguration.loadConfiguration(file);
    
    if (cfg.getConfigurationSection("Nickname") != null) {
      for (String key : cfg.getConfigurationSection("Nickname").getKeys(false)) {
        getObject_nicknames.put("Nickname." + key, cfg.get("Nickname." + key));
      }
    }
    try
    {
  ...
  public void loadData_NickSettings()
  {
    ...
    if (cfg.getConfigurationSection("settings") != null) {
      for (String key : cfg.getConfigurationSection("settings").getKeys(false)) {
        ...
        getObject_nicksettings.put("settings." + key, o);
      }
    }
    if (cfg.getConfigurationSection("inv.settings") != null)
      for (String key : cfg.getConfigurationSection("inv.settings").getKeys(false)) {
        ...
        getObject_nicksettings.put("inv.settings." + key, o);
      }
    Object o;
    if (cfg.getConfigurationSection("inv.items") != null) {
      for (String key : cfg.getConfigurationSection("inv.items").getKeys(false)) {
        ...
        getObject_nicksettings.put("inv.items." + key, "XXX");
      }
      for (??? = getSectionsItems.iterator(); ???.hasNext();) { key = (String)???.next();
        for (String key1 : cfg.getConfigurationSection("inv.items." + key).getKeys(false)) {
          ...
          getObject_nicksettings.put("inv.items." + key + "." + key1, o);
        } }
      String key;
