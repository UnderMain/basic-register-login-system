	...
	public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
		...
		if (sender instanceof Player) {
			...
			if (p.hasPermission("system.admin")) {
				if (args.length != 1) {
					p.sendMessage(TemplateMain.main.pr + "§7Benutze §e/setspawn <Nummer>");
					return true;
				}

				try {
					int number = Integer.parseInt(args[0]);
					TemplateMain.main.lm.setSpawn(number, p.getLocation());
					p.sendMessage(TemplateMain.main.pr + "§7Du hast den Spawn §e" + number + " §7gesetzt");
